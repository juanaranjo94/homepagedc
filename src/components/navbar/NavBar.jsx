      ...
      <nav className=" container m-auto flex items-center justify-between">
        <div data-aos="fade-down" className="logo">
          <Link
            ...
            to="/"
            className={` ${darkMode ? "text-white" : "text-black"} text-3xl sm:text-3xl`}
          >
            Juan <code className="text-yellow-500 text-2xl">&lt;dev&gt;</code> viaje.
          </Link>
        </div>
        <div
          data-aos="fade-down"
        ...
        >
          ...
          >
            ...
            {navItems.map((item) => (
              ...
              >
                <a
                  ...
                  href={`#${item.name}`}
                  className={`uppercase cursor-pointer ${darkMode ? "text-white" : "text-black"} hover:text-yellow-600 font-bold ${
            ...
            <a
              href="#contact"
              className={`${darkMode ? "text-black bg-white" : "text-white bg-black"}  text-[1rem] px-8 py-2 rounded-lg font-bold hover:text-yellow-400 md:m-5 md:block md:mx-auto md:w-fit lg:px-3`}
            >
              CONTACT
            </a>
            <button onClick={toggleDarkMode}>
              {darkMode ? <BiSun className={`${darkMode ? "text-white " : "text-black bg-white"}`}/> : <BiMoon />}
            </button>
